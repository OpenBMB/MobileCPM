...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1, std::pair<int, int> load_image_size = {448, 448}) {
    ...
    // model size and capabilities
    {
        ...
        // TODO: @sgl 这一行需要保留
        new_clip->has_minicpmv_projector = true;
    ...
    if (new_clip->has_vision_encoder) {
        ...
        try {
            if(new_clip->has_llava_projector){
                vision_model.patch_bias = get_tensor(new_clip->ctx_data, "v.patch_embd.proj.bias");
            }
            else if(new_clip->has_minicpmv_projector){
        ...
        try {
            if(new_clip->has_llava_projector){
                vision_model.patch_embeddings    = get_tensor(new_clip->ctx_data, "v.patch_embd.proj.weight");
                vision_model.position_embeddings = get_tensor(new_clip->ctx_data, format("%s.position_embd", "v"));
            }
            else if(new_clip->has_minicpmv_projector){
