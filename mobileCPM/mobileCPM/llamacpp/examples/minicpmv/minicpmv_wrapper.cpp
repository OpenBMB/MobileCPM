...
bool isEmbedValid(uhd_image_embed * embeds) {
...
//    LOG_TEE("%s: image token past: %d\n", __func__, n_past);
//}
int process_image(struct minicpmv_context * ctx_llava, std::vector<std::vector<struct llava_image_embed *>> image_embed_slices, gpt_params * params, int n_cur, std::string system_prompt) {
//    std::string system_prompt;
    int n_past = n_cur;
//    system_prompt = "<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n";
    LOG_TEE("%s: image token past: %d\n", __func__, n_past);
